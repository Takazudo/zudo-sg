...
// must stay in sync with island state and must fall back to state when closed
// natively (Escape / the `cancel`→`close` sequence).
//
 ...
 *   — i.e. a native close (Escape, the close event). Use it to reset the
 *   owning state back to its closed value.
