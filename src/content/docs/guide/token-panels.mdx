...
|---|---|---|
| **Trigger** | **Design Tokens** icon (sliders glyph) in the header — shown on **every** page, docs and styleguide alike | **Preview tokens** button (stacked-frames glyph) in a component detail page's workbench toolbar (`/components/<slug>`); also a "Preview tokens →" button on the `/components/tokens` toolbar |
...
Click the **Preview tokens** button (the stacked-frames glyph) in a component detail page's workbench toolbar, or the "Preview tokens →" button on the `/components/tokens` toolbar, to open this panel.
...
The `/components/tokens` page starts with three static dashboards under **Declared defaults**. The `ui-defaults-light` and `ui-defaults-dark` instances, titled **UI tokens — mode-dependent defaults (light)** and **UI tokens — mode-dependent defaults (dark)**, use `include="mode-dependent"` to show the rows whose declared values differ by mode. The `ui-defaults-shared` instance, titled **UI tokens — shared defaults (mode-independent)**, uses `include="mode-independent"` and `chrome="host"` to show every other row once while following the site theme. They are read-only reference views with grouped palette strips, actual-size spacing rulers, and multiline typography specimens.

The dashboards are isolated from both token panels and from saved tweaks. Editing either panel never changes these declared-default views. The live click-to-copy rows below them are a separate **Live values (host cascade)** area; those rows resolve values from the page's own `:root`, so they can reflect host-cascade values rather than the dashboard's declared UI defaults.
