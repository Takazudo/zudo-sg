...
The `/components/tokens` page starts with two static dashboards under **Declared defaults**: **UI tokens — declared defaults (light)** and **UI tokens — declared defaults (dark)**. They show the declared `@zudo-sg/ui` defaults for each mode, with grouped palette strips, actual-size spacing rulers, and multiline typography specimens. The dashboards are read-only reference views.
