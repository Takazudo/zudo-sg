...
Its **Palette** tab exposes the raw Tier-1 colors as `--palette-{group}-{n}` swatches, with separate groups for `white`, `cool`, `warm`, `brand`, `accent`, `success`, and `danger`. The semantic **Color** tab keeps the unchanged `--color-*` names that point at those palette entries.
