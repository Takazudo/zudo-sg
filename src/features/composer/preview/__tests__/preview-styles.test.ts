...
describe("preview palette scope — structure", () => {
  ...
  it("restores them from the canonical --palette-* rungs, not hard-coded colors", () => {
    ...
    // The preview restoration uses direct `light-dark()` values; an indirection
    // remains acceptable for future semantic aliases, so pin the property
    // instead of requiring one exact declaration syntax.
