...
// `/sg/preview`). Adapted from zzmod's `preview-app.tsx`.
//
// Static-host reality: zfb emits ONE `/sg/preview/index.html`; the query string
// is ignored by static hosting, so every variant iframe loads the SAME HTML.
// The variant to show is therefore resolved CLIENT-SIDE from `location.search`
// (`?slug=button&variant=Variants`), not from SSR props.
//
// This island imports the story registry directly (eager glob), so the variant
// `render()` closures are bundled into the island chunk and run in the browser
// — no per-story lazy loader / codegen needed (the host's eager glob already
// gives us every module).
//
...
import { getStoryBySlug } from "../data/registry";
import { MSG_HEIGHT, MSG_UPDATE_PROPS, isUpdatePropsMessage } from "./messages";
...
function PreviewApp(): JSX.Element {
  ...
  // stories' `render()` ignore props (controls are metadata-only per
  // STORIES.md §4), so this is infrastructure for S7's render-arg binding —
  // kept wired so the channel exists.
  ...
  return <div class="p-hsp-md" data-sg-variant-root>{variantEntry.story.render()}</div>;
}
