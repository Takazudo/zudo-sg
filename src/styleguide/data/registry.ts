// Styleguide data layer — turns the raw eager-glob `storyModules` map into the
// catalog's navigable, slug-addressable model.
//
// Discovery itself lives in `packages/sg-registry.ts` (the eager-glob point —
// see that file's header for the why-it-lives-there rationale). This module is
// pure data shaping over that map: it is SSR-safe (no DOM, no fs, no async) so
// every page (`/sg`, `/sg/[slug]`, `/sg/tokens`) and the preview app can import
// it freely.

import type { StoryCategory, StoryMeta, Story } from "@zudo-sg/ui";
import { storyModules } from "../../../packages/sg-registry";
 ...
 * Contract path (STORIES.md §5): explicit `Story.source` first, else
 * `meta.usage` as the component-level example. No fs/AST extraction is
 * available on this host (zfb's SSG runtime has no `node:fs`, and `?raw`
 * globs are unsupported), so these two are the only promised sources.
 */
