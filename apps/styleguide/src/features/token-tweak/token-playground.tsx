...
// Interactive layer for the `/tokens` reference. The swatch/spacing/type rows
// are SERVER-RENDERED (no-JS users still see the full token reference); this
// island wires the INTERACTIVITY on top via event delegation against the SSR
// DOM:
//
...
//   3. LIVE TWEAK — a button opens the existing zdtp design-token panel (same
//      event the header "Tokens" button dispatches); edits there propagate to
//      every preview iframe via the token-tweak bridge, and — because the
//      swatches use `var(--…)` inline — this page's swatches update live too.
//
// Contract with the SSR markup (pages/tokens.tsx):
...
import { copyText } from "@/features/code-panel/copy-button";
