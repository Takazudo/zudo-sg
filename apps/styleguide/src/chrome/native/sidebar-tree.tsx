...
// Vendored from zudo-doc's host source (src/components/sidebar-tree.tsx).
// The interactive sidebar tree island — filter input, expand/collapse,
// connector lines, active-item highlight. `@/…` imports are repointed to the
// styleguide's vendored copies; the `useActiveSlug` hook additionally listens
// to AFTER_NAVIGATE_EVENT so the active highlight updates on the styleguide's
// soft-nav (the island persists across swaps, where DOMContentLoaded won't fire).
//
...
// Soft-nav signal: the styleguide swaps pages via zfb's client router, which
// dispatches AFTER_NAVIGATE_EVENT ("zfb:after-swap") rather than reloading. The
// vendored useActiveSlug listens to it (in addition to DOMContentLoaded) so the
// active highlight tracks the URL when this island persists across a swap.
import { AFTER_NAVIGATE_EVENT } from "@takazudo/zudo-doc/transitions";
import { smartBreakToHtml } from "./smart-break";
...
function useActiveSlug(nodes: NavNode[], initial?: string): string | undefined {
  ...
  useEffect(() => {
    ...
    update();
    // The styleguide soft-navigates via zfb's client router, which can persist
    // this island's DOM across a swap — `DOMContentLoaded` won't fire then, so
    // the active highlight would go stale. zfb dispatches AFTER_NAVIGATE_EVENT
    // ("zfb:after-swap") on every swap (and once on first load), so we listen to
    // both: DOMContentLoaded for full-load navigations, AFTER_NAVIGATE_EVENT for
    // soft-nav swaps.
