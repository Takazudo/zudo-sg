...
describe("extractTokens", () => {
  ...
  it("extracts tokens from a ternary expression string", () => {
    const result = tokens(
      `const cls = isActive ? "bg-[var(--sg-fg)] text-[color:var(--sg-bg)]" : "text-[color:var(--sg-muted)] hover:underline";`,
    );
    expect(result.has("bg-[var(--sg-fg)]")).toBe(true);
    expect(result.has("text-[color:var(--sg-bg)]")).toBe(true);
    expect(result.has("text-[color:var(--sg-muted)]")).toBe(true);
  ...
  const MUST_KEEP = [
    ...
    "py-vsp-sm",
    "bg-[var(--sg-accent)]",
    "text-[color:var(--sg-on-accent)]",
    "lg:block",
    "xl:hidden",
    "top-[3.5rem]",
    "w-[var(--sg-code-panel-w)]",
    "bg-[#fff]",
    "focus-visible:outline-[color:var(--sg-focus)]",
  ];
...
describe("emitSafelist", () => {
  ...
  it("keeps bracket color candidates through Tailwind compilation", async () => {
    const extracted = extractTokens(
      `const classes = "bg-[var(--sg-accent)] border-[color:var(--sg-border)] text-[color:var(--sg-muted)] focus-visible:outline-[color:var(--sg-focus)] ring-[color:var(--sg-border-strong)]";`,
      new Set(),
    );
    ...
    expect(safelist).toContain("border-[color:var(--sg-border)]");
    expect(safelist).toContain("text-[color:var(--sg-muted)]");
    expect(css).toContain("border-color: var(--sg-border)");
    expect(css).toContain("color: var(--sg-muted)");
    expect(css).toContain("background-color: var(--sg-accent)");
    expect(css).toContain("outline-color: var(--sg-focus)");
    expect(css).toContain("--tw-ring-color: var(--sg-border-strong)");
  });
});
...
describe("gen-safelist against the built package", () => {
  it("dist/safelist.css (once built) contains real catalog/chrome classes", () => {
    ...
    expect(content).toContain("border-[color:var(--sg-border)]");
    expect(content).toContain("text-[color:var(--sg-muted)]");
  });
});
