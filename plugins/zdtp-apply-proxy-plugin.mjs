...
//   zdtp rewrites the first top-level `:root { ... }` block and the first
...
//
// SAME-FILE COALESCING — two routed prefixes share EACH target file (palette +
//   color; and six prefixes on tokens.css). The pinned zdtp (>=0.4.7) groups
//   routing groups by resolved target file before computing and writing, so a
//   complete Apply request preserves every same-file edit and returns one row
//   per file. Downgrading below 0.4.7 reintroduces the historical clobber
//   hazard; keep this dependency pinned. zdtp also attempts reverse-order
//   restoration if a later write fails, but that is not a transaction
//   guarantee; any reported restoreFailures require manual inspection.
...
export function createDevMiddlewareHandler({ rootDir, writeRoot, routing }) {
  ...
  return async function handleApplyRequest(zfbReq) {
    ...
    // Forward the complete payload once so pinned zdtp can coalesce all
    // routing groups by resolved target file before it computes and writes.
