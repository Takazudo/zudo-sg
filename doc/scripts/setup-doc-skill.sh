...
# This nested workspace historically publishes the stable `doc-wisdom` name.
# Deriving from the scoped package name (`@zudo-sg/doc`) would produce an
# invalid skill directory and break existing user-level links.
DEFAULT_SKILL_NAME="doc-wisdom"
LEGACY_DEFAULT_SKILL_NAME="doc-wisdom-wisdom"

echo ""
echo "=== zudo-doc Skill Setup ==="
echo ""

# Skill name is deterministic and matches doc/.gitignore. An explicit override
# is still allowed via the first CLI arg or SKILL_NAME, but never via a prompt.
...
# Migration warning adapted from zudo-doc#3154: detect a stale doubled-name
# directory or link from pre-migration setups and tell the user how to clean it
# up by hand; never delete anything automatically from the user's tree.
...
if [ "$SKILL_NAME" = "$DEFAULT_SKILL_NAME" ] && [ "$DEFAULT_SKILL_NAME" != "$LEGACY_DEFAULT_SKILL_NAME" ]; then
  ...
  if [ -n "$legacy_gitignore" ] ||
    ...
    echo "  This project's stable skill name is:"
...
# Symlink the repository's OWN hand-written skills into the global skills dir.
# Discovery walks the repo-root target directory dynamically. Sources resolve
# through REPO_ROOT so links survive linked-worktree removal. Target-local:
# each target walks only its own
# ".$target/skills/" -- a missing directory is a silent no-op, and there is
# no ".claude" -> ".codex" fallback.
link_tracked_skills() {
  local target="$1"
  local skills_root="$REPO_ROOT/.$target/skills"
  local global_skills_dir="$HOME/.$target/skills"
