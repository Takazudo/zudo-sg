...
Shared UI components live under `packages/demo-ui/src/<component>/`. Use kebab-case component directory names and co-locate the component, story, tests, and optional component MDX docs.

When a component is added or renamed:

- keep exports in `packages/demo-ui/src/index.ts` sorted into the existing category sections;
...
Top-level documentation categories are configured in `doc/zfb.config.ts` through `zudoDoc({ headerNav: [...] })`. Each item should point to a top-level docs directory and use a single directory name for `categoryMatch`, such as `getting-started`, `architecture`, or `development`.
