...
| Tier | Purpose | Source |
| --- | --- | --- |
| Palette | Raw OKLCH values named `--palette-{group}-{n}` | `packages/ui/styles/colors.css` |
| Semantic | `--color-*` roles that point to palette values with `light-dark()` | `packages/ui/styles/colors.css` |
| Component | Scoped overrides when a component truly needs them | component code or CSS |

Palette groups include `white`, `cool`, `warm`, `brand`, `accent`, `success`, and `danger`, with numeric steps ordered from light to dark inside each group.
