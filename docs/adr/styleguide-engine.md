...
**Island rule:** a host `"use client"` component that must hydrate on an
engine route is statically reachable from a host `pages/` file
(`pages/index.tsx` → `_body-end-islands.tsx` already covers the doc-chrome
panels). Stories' components need nothing: catalog thumbs are SSR-only and the
...
### Needs a browser check (structural proof only above)

- `/components/preview?slug=button&variant=Primary` on the packed host: the
  `ConfiguredPreviewApp` island mounts (`html[data-sg-preview-hydrated="1"]`,
  `[data-sg-preview-app="button"]` rendered) and the button shows the UI
  palette (`#2563eb` accent), not the host bundle's `#ff0000`.
- `/components` on the packed host with the `pages/index.tsx` registry import
  in place: clicking the `Counter` thumb increments (island 2 hydrates through
  rule 7).
...
- #664: no host stub is required for hydration; keep `pages/index.tsx`
  importing `_body-end-islands.tsx`.
